...
function getOutputChannel(): vscode.OutputChannel {
	if (!_channel) {
		_channel = vscode.window.createOutputChannel("vscode-dart-helper");
	}
...
export function activate(context: vscode.ExtensionContext) {
	...
	console.log('Congratulations, your extension "vscode-dart-helper" is now active!');

	// The command has been defined in the package.json file
	// Now provide the implementation of the command with registerCommand
	// The commandId parameter must match the command field in package.json
	let disposable = vscode.commands.registerCommand('vscode-dart-helper.generateModel', () => {
		...
		// vscode.window.showInformationMessage('Hello World from vscode_dart_helper!');
	});

	context.subscriptions.push(disposable);

	context.subscriptions.push(
		vscode.commands.registerCommand("vscode-dart-helper.generateModelWatch", () => {
